...
export default function Reports() {
  ...
  return shell(
    <div style={{ width: '100%', maxWidth: 760, animation: 'fadeUp .6s cubic-bezier(.32,.72,0,1) both' }}>
      ...
      <div style={{ ...GLASS, borderRadius: 26, padding: 24 }}>
        ...
        {dates.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '30px 0' }}>
            <div style={{ fontSize: 14, color: ink(.5), marginBottom: 16 }}>{all.length ? 'No report matches that date.' : 'Upload your first CSV to see it here.'}</div>
              ...
              return (
                <div key={d.date} className="hover-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '13px 13px 13px 18px', borderRadius: 999, background: ink(.04), border: '1px solid ' + (isConfirm ? 'rgba(255,59,48,.3)' : ink(.06)) }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, minWidth: 0 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: ink(.3), width: 22, flex: 'none', fontVariantNumeric: 'tabular-nums' }}>{String(i + 1).padStart(2, '0')}</div>
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11.5, color: ink(.5), whiteSpace: 'nowrap' }}>
                  ...
                  {isConfirm ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, flex: 'none' }}>
                      ...
                      <button onClick={() => setConfirmDate(d.date)} title="Delete report" aria-label="Delete report"
                        style={{ ...BTN, width: 32, height: 32, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: ink(.06), color: ink(.45), padding: 0 }}>
                        <Trash />
                      </button>
                    </div>
                  )}
      ...
      <div style={{ textAlign: 'center', fontSize: 11.5, color: ink(.38), marginTop: 22 }}>
